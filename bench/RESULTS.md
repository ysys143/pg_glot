...
## baseline 비교 — lindera 없이 stock PG로는? (recall 포함)

형태소 분석 없이 **순정 PostgreSQL 기능만**으로 같은 MIRACL 셋을 검색한 대조군. lindera
토크나이저(pg_glot)의 가치를 정량화한다. (a) **PG native** = `pg_catalog.simple` config로
BM25 색인(기본 파서 = 공백/스크립트 경계 분리, 형태소 분석 없음). (b) **pg_trgm** = 문자
trigram `word_similarity` 거리(`<<->`, GiST) KNN.

| lang | PG native(simple) BM25 | pg_trgm(word_sim) | **lindera (pg_glot)** |
|---|---|---|---|
|  | NDCG / R@10 | NDCG / R@10 | NDCG / R@10 |
| ko | 0.409 / 0.479 | 0.210 / 0.327 | **0.636 / 0.798** |
| ja | 0.143 / 0.179 | 0.373 / 0.516 | **0.565 / 0.773** |
| zh | 0.011 / 0.017 | 0.234 / 0.364 | **0.459 / 0.646** |

- **native simple**은 공백 분리뿐 → 공백 있는 ko는 부분 작동(R 0.479)하나 공백 없는 ja/zh는
  거의 붕괴(R 0.179 / **0.017**). 형태소·분절 없이는 CJK 검색이 성립하지 않음을 보여준다.
- **pg_trgm**(문자 trigram)은 부분문자열을 잡아 ja/zh에서 native를 크게 앞서나(R 0.516/0.364),
  ko는 음절 블록이 trigram으로 쪼개져 오히려 native보다 낮다(R 0.327 < 0.479) — 언어마다 최선
  baseline이 다르다(ko=native, ja/zh=trgm).
- **lindera는 3개 언어 모두 두 baseline을 큰 폭으로 능가**. recall@10 기준 각 언어 최선 baseline
  대비 ko **+0.319**(0.479→0.798), ja **+0.257**(0.516→0.773), zh **+0.282**(0.364→0.646).
- 재현: `bench/eval_glot.py --config pg_catalog.simple …`(native), `bench/eval_trgm.py {ko,ja,zh}`(pg_trgm).
...
# baseline 대조군(§baseline 비교):
python3 bench/eval_glot.py --lang japanese --config pg_catalog.simple --corpus bench/data/ja/corpus.json --queries bench/data/ja/queries.json --dsn "$DSN"  # PG native
python3 bench/eval_trgm.py ja                         # pg_trgm (ko/zh도 동일)
# 갭 분해(측정 전용): bench/mecab_pretokenize.py, bench/eval_bm25_grid.py, bench/pretokenize_lindera.py
```
