...
### 1.2 문제의식
이 스택을 쓰려면 사용자가 mecab 설치 → textsearch_ko 설치 → pg_textsearch 설치 → pgvector 설치 → RRF SQL 직접 작성을 해야 함. **번거롭다.** 이를 `pg_textsearch_ko`라는 단일 확장으로 "원클릭"화하는 것이 목표.
...
### 2.2 4겹 스택
```
┌─ pg_textsearch_ko (우리가 직접 구현하는 pgrx 확장) ──────────────────┐
...
│     (b) ko_search_hybrid() 등 RRF 하이브리드 SQL                      │  ← 검색 오케스트레이션
└───────────────────────────────────────────────────────────────────────┘
        │ requires                              │ requires
        ▼                                       ▼
...
- **쿼리**: 질의텍스트 → BM25(pg_textsearch) top-k + 질의벡터 → Dense(pgvector) top-k → `ko_search_hybrid` RRF 융합 → 최종 순위
...
```
pg_textsearch_ko/  (모노레포, Cargo workspace)   ※ 이름은 스캐폴딩 시 확정
├── crates/
│   └── korean-tokenizer/   순수 Rust lib: lindera 래퍼 + analyzer trait (+opt-in kiwi)
│                            → crates.io 게시, Postgres 밖에서도 재사용
├── extensions/
│   ├── (A) pg_tsvector_ko    pgrx: 커스텀 TS parser → korean config. (의존: korean-tokenizer)
│   │                          순수 한국어 FTS/GIN/ts_rank만 원하면 이것만 설치. textsearch_ko 대체물.
│   └── (B) pg_textsearch_ko  pgrx: BM25 + RRF 하이브리드. (requires: A + pg_textsearch + vector)
│                              "깔면 기존 pgvector 위에 한국어 BM25+RRF가 얹힘"
├── bakeoff/  docs/  ...
```

- **설치 UX는 단일 명령**: `CREATE EXTENSION pg_textsearch_ko CASCADE` → requires 체인으로 pg_tsvector_ko + pg_textsearch + vector 자동 생성. 분리해도 풀스택 사용자 비용 0, 순수 FTS 사용자는 pg_tsvector_ko만.
...
- **이름 확정(2026-06-19, 2계층)**: A=`pg_tsvector_ko`(토크나이저/korean config), B=`pg_textsearch_ko`(BM25+RRF). **별도 `pg_hybrid_ko` 폐기 — RRF는 B에 흡수.** 공유 crate=`korean-tokenizer`, repo=`pg_textsearch_ko`(namesake).
...
- **결정**: pg_textsearch·pgvector는 흡수하지 않고 `requires`로 의존. 우리는 글루(TS parser + RRF SQL)만 직접 구현. 진짜 원클릭 체감은 Docker 프리빌드 이미지로 `CREATE EXTENSION pg_textsearch_ko CASCADE` 한 줄.
...
- **결정**: B(pg_textsearch_ko)에 RRF를 **2층 API**로 제공.
  - **`ko_rrf(bm25_ranked, dense_ranked, k)`** — 일반 fusion 프리미티브. (id, rank) 리스트만 받아 융합. 백엔드·언어·스키마 무관, 커스텀 스키마/외부(pg_aidb)도 재사용.
  - **`ko_search_hybrid(rel, key_col, text_col, vec_col, q_text, q_vec, k)`** — pgvector 편의 어댑터. 내부에서 BM25 leg(korean config+pg_textsearch) + dense leg(pgvector `<=>`) 실행 후 `ko_rrf` 융합. 흔한 케이스 원콜. 연구 Phase 7 DB-side RRF CTE를 함수로 포장.
- **근거**: 연구가 DB-side RRF의 latency 우위(왕복 1회) 입증. 2층 분리로 casual=원콜 / power=ko_rrf+CTE 자유조합(스키마 과적합 회피).
...
- **결정**: **2계층**(A=`pg_tsvector_ko` 토크나이저/config, B=`pg_textsearch_ko` BM25+RRF) + 공유 `korean-tokenizer` 크레이트. 별도 하이브리드 확장(`pg_hybrid_ko`)은 **폐기**(RRF를 B에 흡수). **v0.1 = 모노레포(Cargo workspace), 안정화 후 별도 리포 분리.** (§2.4)
- **2계층 근거(2026-06-19 갱신)**: `CREATE EXTENSION pg_textsearch_ko CASCADE`가 A+pg_textsearch+vector를 자동 생성 → 분리해도 설치 UX 동일(다운사이드 0), 순수 FTS 수요는 A만 설치. dense는 pgvector 표준 인터페이스에 작성 → pg_acorn/pg_cuvs/pgvectorscale/vchord와 자동 호환(dense seam 불필요). 별도 하이브리드 확장은 pg_aidb의 hybrid와 중복이라 폐기.
...
| 구성요소 | 라이선스 | 흡수/의존 | 비고 |
|---|---|---|---|
| 확장 본체(pg_textsearch_ko) | **PostgreSQL License**(또는 BSD 2-clause) | — | 공개+상업 OK |
...
2. **모노레포 스캐폴딩**: Cargo workspace + `crates/korean-tokenizer` + `extensions/{pg_tsvector_ko, pg_textsearch_ko}`. control 파일(B: `requires = 'pg_tsvector_ko, pg_textsearch, vector'`), pg_regress 뼈대. 별도 하이브리드 확장 없음(RRF는 B에 포함). (§2.4)
3. **analyzer seam (Layer A)**: `korean-tokenizer` 크레이트 — trait(`tokenize(text) -> Vec<Token>`) + lindera 구현(+opt-in kiwi feature). `pg_tsvector_ko` 확장 — 커스텀 TS parser → `korean` config 등록.
...
4. **하이브리드 API**: Phase7 RRF SQL 이식 → `ko_search_hybrid()` 시그니처 확정.
...
| 토크나이저/인덱스 재현성(사전=인덱스 정의의 일부) | **높음** | `pg_tsvector_ko_dictionary_version()` 노출 + 사전 빌드 해시, 사전 변경 시 REINDEX 문서/경고, 업그레이드 스크립트. |
| lextype 토큰 클래스 설계 미흡 → 랭킹 손상 | 중간 | 한국어 lexeme·Latin·숫자·혼합alnum·URL 등 의도적 분류, `ts_debug` 골든, ADD MAPPING 안정·문서화. |
| `ko_search_hybrid(rel,key_col)` 동적SQL + SECURITY DEFINER 풋건 | 중간 | definer 회피, `regclass`/`format('%I')`/`SET search_path=pg_catalog,pg_temp`. `ko_rrf` 주력·어댑터 experimental. |
