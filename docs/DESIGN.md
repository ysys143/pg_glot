...
- **현황(2026-06 갱신)**: Kiwi opt-in은 **미구현(follow-on)**. lindera가 ko BM25 NDCG 0.636으로
  research MeCab(0.6385)와 통계적 parity 도달(`bench/RESULTS.md`) → Kiwi opt-in의 실익이 작아
  후순위로 내려감. 대신 pluggable seam(`text_config` regconfig + `Analyzer` trait)은 **CJK 3개
  언어(ko/ja/zh, lindera 사전 3종)로 실증**됨. seam은 열려 있어 Kiwi 추가 시 동일 경로로 들어온다.

### D6. 하이브리드 = DB-side RRF (3단 API: 프리미티브 → SRF 어댑터 → 표현 연산자)
- **결정**: B(pg_glot_hybrid)에 RRF를 **3층 API**로 제공.
  - **`glot.rrf(bm25_ranked, dense_ranked, k)`** — 일반 fusion 프리미티브. (id, rank) 리스트만 받아 융합. 백엔드·언어·스키마 무관, 커스텀 스키마/외부(pg_aidb)도 재사용.
  - **`glot.hybrid(rel, key_col, text_col, vec_col, q_text, q_vec, k)`** — pgvector 편의 어댑터(SRF). 내부에서 BM25 leg(korean config+pg_textsearch) + dense leg(pgvector `<=>`) 실행 후 `glot.rrf` 융합. 흔한 케이스 원콜. 연구 Phase 7 DB-side RRF CTE를 함수로 포장.
  - **`glot.rank(text_col, vec_col, q_text, q_vec, k, per_leg)`** (flagship 표현) — `set_rel_pathlist_hook` + CustomScan(`GlotHybrid`). `SELECT … FROM docs ORDER BY glot.rank(body, emb, q_text, q_vec) DESC LIMIT k`를 인식해 내부적으로 `glot.hybrid`(두 인덱스 leg + RRF)에 위임하고 행을 emit. 표면은 pgvector KNN과 동일(`FROM docs` + 실제 컬럼 참조 + 리터럴 질의) — 하이브리드의 4-ary(텍스트 컬럼·벡터 컬럼·텍스트 질의·벡터 질의)를 whole-row/생성자로 묶지 않고 평범한 함수 인자로 노출한 것이 핵심 미감. **`shared_preload_libraries='pg_glot_hybrid'` 필요**(hook은 `_PG_init`에서 등록; 없으면 planner가 재작성 못 해 마커 함수 직접 평가=ts_rank_cd+dense 합의 비-RRF 폴백). (Codex 구현; `#[pg_test]`로 EXPLAIN의 GlotHybrid 노드 + `glot.hybrid` parity 검증.) [[project-identity-lexical-rrf]]
...
3. **델타(+0.014) 유의성 미검정**(bootstrap CI/p-value 없음) → 안전한 결론은 "lindera가 베이스라인보다 **나쁘지 않다**". **(Codex 리뷰)** 공개 v0.1 전 **lindera+실제 pg_textsearch로 Phase7 품질 재현을 release gate**로(통과 전 NDCG parity 홍보 금지); bootstrap CI·per-query win/loss·10K/100K 재측정 추가.
   ...
   - **2026-06-22 후속 — POS accept-list로 사실상 parity 달성**: 위에서 "POS 필터 무효"라 한 판정이 **틀렸다**(잘못된 넓은 allowlist). MeCab same-harness로 갭을 재분해하니 **토크나이저 분절은 lindera ≈ MeCab**(무필터 0.606 vs 0.6038)이고 **진짜 레버는 정확한 POS accept-list**였다(`NNG,NNP,NNB,NNBC,NR,VV,VA,MM,MAG,XSN,XR,SH,SL`). lindera·MeCab 모두 같은 ko-dic POS 체계라 그대로 이식 → ko BM25 NDCG **0.636**(research 0.6385의 99.7%, MeCab 0.633 초과). **외부 의존·토크나이저 교체 없이 정체성(순수 Rust 임베드) 그대로 달성.** lindera baseline을 **0.636**로 상향. (가설→측정→재측정 자기수정의 사례) — 후속으로 A1의 english_stem은 CJK 라틴(음역)엔 무의미함이 측정돼 제거(simple)·lextype 단순화; ja/zh는 POS·stopword·stem 모두 무효라 현 방식 유지(`bench/RESULTS.md`).
