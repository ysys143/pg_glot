...
`pg_glot_hybrid` itself is under the **PostgreSQL License** (see `../LICENSE`).
...
|---|---|---|---|
| **all statically-linked Rust crates** (pgrx, serde, lindera tree, …) | compiled into `pg_glot.so` / `pg_glot_hybrid.so` | MIT / Apache-2.0 / BSD / ISC / … (permissive) | **`RUST_CRATES.html`** (auto-generated by `cargo-about` from `Cargo.lock` — the authoritative, complete list) |
