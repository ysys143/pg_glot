...
**pg_glot**은 PostgreSQL에 **CJK(한국어·일본어·중국어) 전문검색과 BM25 + dense 하이브리드(RRF)
검색**을 더한다(pgvector 위). 형태소·분절 엔진은 순수 Rust(lindera + 임베드 사전)라 외부 사전
설치가 필요 없다.

독립적으로 골라 쓸 수 있는 확장 둘로 제공된다:

- **`pg_glot`** — CJK 토크나이저와 `korean` / `japanese` / `chinese` 텍스트서치 config(일반
  PostgreSQL 전문검색), 그리고 `glot.rrf` 융합 프리미티브.
- **`pg_glot_hybrid`** — BM25 색인과 BM25 + dense 하이브리드 검색(`glot.rank`, `glot.hybrid`).
  `pg_glot` + `pg_textsearch` + `pgvector` 위에 얹힌다.

> **상태** — 두 확장 모두 동작. MIRACL dev로 BM25/RRF 실측([`bench/RESULTS.md`](bench/RESULTS.md)).
> ko가 가장 엄밀하게 검증됨(POS ablation·research parity); ja/zh도 측정값은 있으나 제품 품질은
> ko 수준으로 검증 전. 설계·의사결정 전문은 [`docs/DESIGN.md`](docs/DESIGN.md).

## 구성요소

하나의 모노레포(Cargo workspace), 단 각 확장은 깨끗한 경계를 가져 독립 설치 가능하다:

| 구성요소 | 역할 | 의존 |
|---|---|---|
| `crates/glot-tokenizer` | 순수 Rust CJK 토크나이저 (lindera + 임베드 ko-dic / IPADIC / CC-CEDICT) | — |
| `extensions/pg_glot` | 커스텀 TS parser → `korean` / `japanese` / `chinese` config; `glot` 스키마(`glot.rrf`) 소유 | glot-tokenizer |
| `extensions/pg_glot_hybrid` | CJK BM25 + RRF 하이브리드 — `glot.rank` custom scan(`ORDER BY … LIMIT`) + `glot.hybrid` SRF | pg_glot + pg_textsearch + pgvector |

## 설치

**필요한 것만 설치한다** — `pg_textsearch`·`pgvector`는 흡수가 아니라 의존(`requires`)이라 각자
일정으로 업그레이드한다.
...
| RRF 융합 프리미티브(`glot.rrf`) | `CREATE EXTENSION pg_glot;` | — (`glot` 스키마는 `pg_glot`에 동봉) |
| PostgreSQL 밖에서 토크나이저만 | `glot-tokenizer` 크레이트 의존 | — |

하이브리드 경로는 `shared_preload_libraries = 'pg_textsearch, pg_glot_hybrid'`가 필요하다
(프리빌드 Docker 이미지는 설정 완료). `pg_textsearch`는 BM25용, `pg_glot_hybrid`는 `glot.rank`
custom-scan hook 등록용이다. 경계가 깨끗해 추후 리포 분리는 기계적이다.
...
-- 또는 미리 만든 id 리스트를 RRF 프리미티브로 직접 융합 (pg_glot에 동봉)
...
`GlotHybrid` custom-scan hook을 등록하기 때문(프리빌드 Docker 이미지는 설정 완료). hook이 없으면
planner가 질의를 재작성하지 못해 `glot.rank`가 **비-RRF 점수로 폴백**한다. `body`/`emb`는 진짜
컬럼 참조, 질의 둘은 리터럴; 테이블에 BM25 인덱스 필요(HNSW도 있으면 dense가 인덱스, 없으면
exact 스캔).

**`glot.hybrid`(명시적 형태)** 는 첫 인자(`'docs'`, `regclass`)가 대상 테이블, 그 뒤 셋은
키/본문/벡터 컬럼이다. 테이블에 BM25 인덱스(`text_config` 일치)와 벡터 인덱스가 있어야 하고 키
컬럼은 `bigint`. preload hook 없이도 동작. 필요하면 스키마 한정: `'myschema.docs'`.

`'public.korean'`(및 `'korean'`)을 `japanese`/`chinese`로 바꾸면 언어가 전환된다.

## 검색 품질 (MIRACL dev, 실측)

`bench/`로 실제 `pg_glot` + `pg_textsearch` BM25를 측정했다. 상세·한계·재현은
...
**lindera 없이 stock PostgreSQL로는?** (형태소 분석 없는 대조군, recall@10)
...
공백 없는 ja/zh에서 native `simple`은 거의 붕괴(zh R 0.017), pg_trgm도 부분문자열만 잡는다.
